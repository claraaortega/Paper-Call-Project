...
#
# Salida:
#   - data/output/final_author_topics_analysis.csv 
##############################################################################
...
OUT_FINAL <- file.path(DIR_OUTPUT_VENTANA, "final_author_topics_analysis.csv")
...
# A. Separamos los IDs pero guardamos el original
...
aggregate_count <- function(df_long, group_col_name, file_suffix, top_n_limit = 10) {
  ...
  author_items_compact <- df_long %>%
    ...
    group_by(email, author_name, all_au_ids) %>%
    summarise(!!paste0("top_", file_suffix) := paste(label, collapse = " | "), .groups = "drop")
