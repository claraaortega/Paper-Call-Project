##############################################################################
# Script: 14_build_author_text_profile_bruto.R
# Propósito: 
...
FILE_OUT <- file.path(DIR_SALIDA, "author_text_profiles_bruto.csv")
...
df_keywords <- df_ugr_pubs %>%
  ...
  summarise(
    repeated_keywords = paste(rep(keywords, kw_count), collapse = "; "),
    .groups = "drop"
  )
